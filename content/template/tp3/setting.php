...
function template_setting_view() {
    $storage = TemplateStorage::getInstance('tp3');
    $themeColor = (string) $storage->getValue('theme_color');
    if ($themeColor === '') $themeColor = '#f59e0b';
    $bannerText = (string) $storage->getValue('banner_text');
?>

<div class="popup-inner">
<form class="layui-form" id="tp3Form" lay-filter="tp3Form">
    ...
    <div class="popup-section">
        ...
        <div class="layui-form-item" style="margin-top: 16px;">
            ...
            <div class="layui-input-block">
                <input type="text" class="layui-input" name="theme_color" value="<?php echo htmlspecialchars($themeColor, ENT_QUOTES, 'UTF-8'); ?>" placeholder="#f59e0b">
            </div>
        </div>
...
<div class="popup-footer">
    <button type="button" class="popup-btn popup-btn--default" id="tp3CancelBtn">取消</button>
    <button type="button" class="popup-btn popup-btn--primary" id="tp3SubmitBtn"><i class="layui-icon layui-icon-ok"></i> 保存配置</button>
</div>
...
(function(){
    layui.use(['layer','form'], function(){
        ...
        $('#tp3CancelBtn').on('click', function(){
            var index = parent.layer.getFrameIndex(window.name);
            parent.layer.close(index);
        });

        $('#tp3SubmitBtn').on('click', function(){
            var $btn = $(this);
            ...
            $.ajax({
                ...
                data: $('#tp3Form').serialize() + '&_action=save_config&name=tp3',
                dataType: 'json',
                success: function(res){
                    if (res.code === 0 || res.code === 200) {
                        if (res.data && res.data.csrf_token) {
                            $('#tp3Form input[name=csrf_token]').val(res.data.csrf_token);
                        }
