...
function template_setting_view() {
    $storage = TemplateStorage::getInstance('tp1');
...
<form class="layui-form" id="tp1Form" lay-filter="tp1Form">
    <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars((string) Csrf::token(), ENT_QUOTES, 'UTF-8'); ?>">

    <div class="popup-section">
        <blockquote class="layui-elem-quote">
            演示模板 #1 —— 用于测试模板配置保存 / 卸载流程。
        </blockquote>

        <div class="layui-form-item" style="margin-top: 16px;">
            <label class="layui-form-label">主题色</label>
            <div class="layui-input-block">
                <input type="text" class="layui-input" name="theme_color" value="<?php echo htmlspecialchars($themeColor, ENT_QUOTES, 'UTF-8'); ?>" placeholder="#6366f1">
            </div>
        </div>
...
<div class="popup-footer">
    <button type="button" class="popup-btn popup-btn--default" id="tp1CancelBtn">取消</button>
    <button type="button" class="popup-btn popup-btn--primary" id="tp1SubmitBtn"><i class="layui-icon layui-icon-ok"></i> 保存配置</button>
</div>
...
(function(){
    layui.use(['layer','form'], function(){
        ...
        $('#tp1CancelBtn').on('click', function(){
            var index = parent.layer.getFrameIndex(window.name);
            parent.layer.close(index);
        });

        $('#tp1SubmitBtn').on('click', function(){
            var $btn = $(this);
            ...
            $.ajax({
                ...
                data: $('#tp1Form').serialize() + '&_action=save_config&name=tp1',
                dataType: 'json',
                success: function(res){
                    if (res.code === 0 || res.code === 200) {
                        if (res.data && res.data.csrf_token) {
                            $('#tp1Form input[name=csrf_token]').val(res.data.csrf_token);
                        }
