...
var GoodsDetail = (function () {
    ...
    function bindEvents() {
        ...
        applyDetailCoupon = function (code) {
            if (!currentSpec && specs.length > 1) { layui.layer.msg('请先选择规格再应用券'); return; }
            var sp = currentSpec || specs[0];
            if (!sp) { layui.layer.msg('商品无规格'); return; }
            var qty = parseInt($('#qtyInput').val()) || 1;
            ...
            $.post('?c=coupon&a=check', {
                ...
                if (res.code !== 200) {
                    if (isRevalidate) {
                        ...
                        $('#detailCouponApplied').hide();
                        layui.layer.msg(res.msg || '当前金额不满足券使用门槛，已取消使用');
                    } else {
                ...
                $('#detailCouponApplied')
                    ...
                    .show();
                if (!isRevalidate) layui.layer.msg('优惠券已应用');
            }, 'json').fail(function () { layui.layer.msg('网络异常'); });
        };
            ...
            var idx = layui.layer.open({
                ...
                yes: function (layerIdx, layero) {
                    ...
                    if (!isGuest) {
                        ...
                        if (!$picked.length) { layui.layer.msg('请选择收货地址'); return; }
                        ...
                        if (!g.recipient || !g.mobile || !g.province || !g.city || !g.district || !g.detail) {
                            layui.layer.msg('请填写完整的收货地址'); return;
                        }
                        if (!/^1\d{10}$/.test(g.mobile)) { layui.layer.msg('请输入正确的手机号'); return; }
