...
if (!function_exists('mpay_get_channel_config')) {
    require_once __DIR__ . '/mpay.php';
}

function mpay_show_h(string $v): string
{
    return htmlspecialchars($v, ENT_QUOTES, 'UTF-8');
}

function mpay_show_json(array $payload): void
{
...
function mpay_show_base64url_decode(string $raw): string
{
    if ($raw === '') {
        return '';
    }
...
function mpay_show_status_text(string $status, bool $isRecharge): string
{
    if ($isRecharge) {
        $map = [
...
function mpay_show_channel_label(string $paymentCode): string
{
    $map = [
        'mpay_alipay' => '支付宝',
        'mpay_wxpay' => '微信支付',
    ...
    return $map[$paymentCode] ?? '码支付';
}

function mpay_show_channel_logo(string $paymentCode): string
{
    $map = [
        'mpay_alipay' => '/content/plugin/mpay/alipay.png',
        'mpay_wxpay' => '/content/plugin/mpay/wxpay.png',
        'mpay_qqpay' => '/content/plugin/mpay/qqpay.png',
    ];
    return $map[$paymentCode] ?? '/content/plugin/mpay/alipay.png';
}

function mpay_show_trade_context(string $orderNo): array
{
    ...
    if ($isRecharge) {
        ...
        if ((string) ($recharge['payment_plugin'] ?? '') !== 'mpay') {
            throw new RuntimeException('该订单不是码支付支付');
        }
...
function mpay_show_decode_payload(string $payloadRaw): array
{
    ...
    $decoded = mpay_show_base64url_decode($payloadRaw);
    if ($decoded === '') {
        return [];
    }
...
function mpay_show_fetch_gateway_payload(string $orderNo, string $paymentCode, int $amountRaw): array
{
    ...
    try {
        $params = mpay_build_trade_params($conf, [
            'order_no' => $orderNo,
    ...
    if (($res['ok'] ?? false) !== true) {
        return ['payload' => [], 'error' => (string) ($res['msg'] ?? '码支付接口请求失败')];
    }

    $ret = (array) ($res['data'] ?? []);
    if ((string) ($ret['code'] ?? '') !== '1') {
        return ['payload' => [], 'error' => (string) ($ret['msg'] ?? '码支付下单失败')];
    }
    ...
    if ($payload['payurl'] === '' && $payload['qrcode'] === '' && $payload['urlscheme'] === '') {
        return ['payload' => [], 'error' => '码支付接口未返回可用支付地址'];
    }

    return ['payload' => $payload, 'error' => ''];
}

function mpay_show_entry_link(array $payload): string
{
...
function mpay_show_qr_text(array $payload): string
{
    $qr = trim((string) ($payload['qrcode'] ?? ''));
    if ($qr !== '') {
        return $qr;
...
try {
    $ctx = mpay_show_trade_context($orderNo);
} catch (Throwable $e) {
    http_response_code(403);
    echo mpay_show_h($e->getMessage());
    exit;
}
...
if (trim((string) ($_GET['action'] ?? '')) === 'status') {
    mpay_show_json([
        ...
        'data' => [
            'paid' => $isPaid,
            'status' => $status,
            'status_text' => mpay_show_status_text($status, $isRecharge),
...
$payError = '';
$payload = mpay_show_decode_payload((string) ($_GET['p'] ?? ''));
if (!$isPaid && $payload === []) {
    $fallback = mpay_show_fetch_gateway_payload($orderNo, $paymentCode, $amountRaw);
...
$qrText = mpay_show_qr_text($payload);
$entryLink = mpay_show_entry_link($payload);
...
$statusText = mpay_show_status_text($status, $isRecharge);
$channelLabel = mpay_show_channel_label($paymentCode);
$channelLogo = mpay_show_channel_logo($paymentCode);
    ...
    <div class="card">
        <section class="hero">
            <div class="hero-media">
                <div class="hero-logo">
                    <img src="<?= mpay_show_h($channelLogo) ?>" alt="<?= mpay_show_h($channelLabel) ?>">
                </div>
            </div>
            <div class="hero-main">
                <span class="hero-tag">码支付收银台 · <?= mpay_show_h($channelLabel) ?></span>
                <h1 class="hero-title"><?= mpay_show_h($title) ?></h1>
                <p class="hero-sub">请在有效时间内完成支付，支付成功后页面会自动刷新并跳转结果页。</p>
                <div class="hero-amount"><?= mpay_show_h($sym . $money) ?></div>
                <div class="hero-order">订单号：<?= mpay_show_h($orderNo) ?></div>
            </div>
        </section>
        ...
        <section class="body">
            <?php if ($isPaid): ?>
                ...
                <div class="btns">
                    <a class="btn btn-primary" href="<?= mpay_show_h($redirectUrl) ?>">查看结果</a>
                </div>
            <?php elseif ($payError !== ''): ?>
                <p class="alert alert-err">创建支付失败：<?= mpay_show_h($payError) ?></p>
                <div class="btns">
                    <a class="btn" href="<?= mpay_show_h($redirectUrl) ?>">返回</a>
                </div>
            <?php else: ?>
                <?php if ($isSubmitPostMode): ?>
                    ...
                    <div class="guide" style="margin-top:12px;">
                        <h3>支付跳转</h3>
                        <ol class="steps">
                            <li>系统将通过表单 POST 提交订单数据到码支付网关。</li>
                            <li>若未自动跳转，请点击下方“继续前往支付”。</li>
                            <li>支付完成后会自动返回并更新订单状态。</li>
                        </ol>
                    </div>
                    <form id="mpaySubmitForm" method="post" action="<?= mpay_show_h($submitTarget) ?>">
                        <?php foreach ($submitFields as $k => $v): ?>
                            <input type="hidden" name="<?= mpay_show_h((string) $k) ?>" value="<?= mpay_show_h((string) $v) ?>">
                        <?php endforeach; ?>
                    </form>
                    <div class="btns">
                        <button type="button" class="btn btn-primary" id="mpayManualSubmitBtn">继续前往支付</button>
                    </div>
                <?php else: ?>
                    <div class="grid">
                        <div class="qr-panel">
                            <div class="qr-box" id="mpayQrBox"></div>
                        ...
                        <div class="guide">
                            ...
                            <div class="meta">
                                ...
                                <span class="meta-value" id="payStatusText"><?= mpay_show_h($statusText) ?></span>
                            </div>
                    ...
                    <?php if ($entryLink !== ''): ?>
                    <div class="btns">
                        <a class="btn btn-primary" href="<?= mpay_show_h($entryLink) ?>">立即支付</a>
                    </div>
                    <?php endif; ?>
                <?php endif; ?>
            <?php endif; ?>
            ...
            <?php if (!$isRecharge && ($orderItems !== [] || $createdAt !== '')): ?>
                <div class="order-info">
                    <div class="order-head">
                        ...
                        <?php if ($createdAt !== ''): ?>
                            <span class="order-time">下单时间：<?= mpay_show_h($createdAt) ?></span>
                        <?php endif; ?>
                    </div>
                    <?php if ($orderItems !== []): ?>
                        <?php foreach ($orderItems as $it): ?>
                            ...
                            <div class="order-item">
                                <img class="order-cover" src="<?= mpay_show_h($itemCover) ?>" alt="">
                                <div>
                                    <p class="order-name"><?= mpay_show_h($itemTitle !== '' ? $itemTitle : '商品') ?></p>
                                    <?php if ($itemSpec !== ''): ?>
                                        <div class="order-spec">规格：<?= mpay_show_h($itemSpec) ?></div>
                                    <?php endif; ?>
                                </div>
...
<script>
(function () {
    var form = document.getElementById('mpaySubmitForm');
    var manualBtn = document.getElementById('mpayManualSubmitBtn');
    if (!form) {
        return;
    }
...
(function () {
    ...
    var qrBox = document.getElementById('mpayQrBox');
    ...
    function poll() {
        ...
        var url = '/?plugin=mpay&action=status&order_no=<?= rawurlencode($orderNo) ?>';
        fetch(url, { credentials: 'same-origin' })
            ...
            .then(function (res) {
                ...
                if (res.data.paid) {
                    ...
                    window.location.href = res.data.redirect_url || '<?= mpay_show_h($redirectUrl) ?>';
                    return;
                }
