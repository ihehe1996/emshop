 ...
 *
 * 本文件的作用：
 *   1. 启动一个 HTTP API 服务（端口可配置）—— 供后台"Swoole监控"页面查询状态
 ...
 *   php swoole/server.php start --port=9602   指定端口启动（覆盖后台配置）
...
define('SW_DEFAULT_HOST', '0.0.0.0');  // 默认监听地址，0.0.0.0 表示所有网卡都能访问
define('SW_DEFAULT_PORT', 9601);       // 默认监听端口

// 监听地址/端口解析优先级：
//   1) 命令行参数 --host= / --port=
//   2) 数据库配置项 swoole_api_url（读取其中的 port）
//   3) 默认值 0.0.0.0:9601
 ...
 *
 * 这里不依赖 init.php，避免在 worker fork 之前加载整套业务框架。
 */
